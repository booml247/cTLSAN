...
class Model(object):
  ...
  def build_model(self, cate_list):
    ...
    if self.config['concat_time_emb'] == True:
      ...
      h_emb = tf.keras.layers.Dense(self.config['hidden_units'])(h_emb)
    else:
      t_emb = tf.keras.layers.Dense(self.config['hidden_units'], activation=tf.nn.tanh)(tf.expand_dims(self.hist_t, -1))
      h_emb += t_emb
...
def feedforward(inputs,
  ...
  with tf.variable_scope(scope, reuse=reuse):
    ...
    outputs = tf.keras.layers.Conv1D(filters=num_units[0], kernel_size=1, activation='relu', use_bias=True)(inputs)


    # Readout layer
    # params = {"inputs": outputs, "filters": num_units[1], "kernel_size": 1,
    #       "activation": None, "use_bias": True}
    outputs = tf.keras.layers.Conv1D(filters=num_units[1], kernel_size=1, activation=None, use_bias=True)(inputs)

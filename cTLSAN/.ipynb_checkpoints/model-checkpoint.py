...
class Model(object):
    def __init__(self, config, item_cate_list, description_embedding):
        self.config = config
        ...
        self.build_model(item_cate_list, description_embedding)
        self.init_optimizer()
    ...
    def init_placeholders(self):
        ...
        # [bs, emb_size] item review
        self.r_emb = tf.placeholder(tf.float32, [None,None])
        ...
        # [bs, sl, emb_size] history item review
        self.hist_r_emb = tf.placeholder(tf.float32, [None, None, None])
        self.hist_r_new_emb = tf.placeholder(tf.float32, [None, None, None])
    ...
    def build_model(self, item_cate_list, description_embedding):
        ...
        i_emb = tf.concat([i_emb, i_cate_emb, self.r_emb], -1)
        i_b = tf.gather(item_b, self.i)
        # all items embedding
        all_cate_emb = tf.nn.embedding_lookup(cate_emb, item_cate_list)
        all_emb = tf.concat([item_emb, all_cate_emb, self.description_embedding], -1)
        ...
        h_emb = tf.multiply(
            tf.concat([h_emb, h_cate_emb, self.hist_r_emb], -1),
        ...
        h_emb_new = tf.concat([h_emb_new, h_cate_emb_new, self.hist_r_new_emb], -1)
    ...
    def train(self, sess, batch, lr, add_summary=False):
        ...
        input_feed = {
            self.u: batch[0],
            self.u_cate: batch[11],
            self.r_emb: batch[1],
            self.i: batch[2],
            self.y: batch[3],
            self.hist_r_emb: batch[4],
            self.hist_i: batch[5],
            self.hist_r_new_emb: batch[6],
            self.hist_i_new: batch[7],
            self.hist_t: batch[8],
            self.sl: batch[9],
            self.sl_new: batch[10],
            self.lr: lr,
    ...
    def eval_auc(self, sess, batch):
        ...
        res1 = sess.run(self.logits, feed_dict={
            self.u: batch[0],
            self.u_cate: batch[12],
            self.r_emb: batch[1],
            self.i: batch[2],
            self.hist_r_emb: batch[5],
            self.hist_i: batch[6],
            self.hist_r_new_emb: batch[7],
            self.hist_i_new: batch[8],
            self.hist_t: batch[9],
            self.sl: batch[10],
            self.sl_new: batch[11],
        ...
        with tf.variable_scope("long_term"):
            for i in range(num_blocks):
                with tf.variable_scope("num_blocks0_{}".format(i)):
                    ...
                    enc = tf.expand_dims(tf.keras.layers.Dense(num_units)(enc), axis=1)
...
def linear(args, output_size, bias, bias_start=0.0, scope=None, squeeze=False, wd=0.0, keep_prob=1.0,
    ...
    if keep_prob < 1.0:
        ...
        flat_args = [tf.cond(
            is_training,
            lambda arg=arg: tf.nn.dropout(arg, rate=keep_prob),
            lambda arg=arg: arg
        ) for arg in flat_args]

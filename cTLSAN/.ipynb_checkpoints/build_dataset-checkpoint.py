...
for dataset in dataset_list:
    ...
    for reviewerID, hist in reviews_df.groupby('reviewerID'):
        ...
        # Encode all reviews
        ## review_embeddings is a numpy array of shape (len(review_list), embedding_dim)
        review_embeddings = text_model.encode(cleaned_review_list, show_progress_bar=False, device='cuda', batch_size=16)
        ...
        neg_review_embeddings = text_model.encode(neg_reviews, show_progress_bar=False, device='cuda', batch_size=16)
        ...
        for t in tim_list_session:
            ...
            new_review = review_embeddings[i:i+count, :]
            ...
            if t == tim_list_session[0]:
                ...
                if i+count < valid_length-1:
                    ...
                    train_set.append((reviewerID, pre_review_copy, pre_session_copy, new_review, new_session, pre_time_emb, review_embeddings[i+count, :], pos_list[i+count], 1, now_cate))
                    train_set.append((reviewerID, pre_review_copy, pre_session_copy, new_review, new_session, pre_time_emb, neg_review_embeddings[i+count, :], neg_list[i+count], 0, now_cate))
                    pre_session.extend(new_session)
                    pre_time.extend(new_time)
                    pre_cates.extend(new_cates)
                    pre_review.extend(new_review)
                    ...
                    pos_neg_review = (review_embeddings[neg_index, :], neg_review_embeddings[neg_index, :])
    ...
    # Upload to S3
    s3_key = f'cTLSAN_input_data/{dataset}_dataset.pkl'

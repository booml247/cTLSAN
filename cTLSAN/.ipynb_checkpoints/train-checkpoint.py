...
# Network parameters
tf.app.flags.DEFINE_integer('hidden_units', 448, 'Number of hidden units in each layer')
...
tf.app.flags.DEFINE_integer('userid_embedding_size', 416, 'User id feature size')
tf.app.flags.DEFINE_integer('cateid_embedding_size', 32, 'Category id feature size')
tf.app.flags.DEFINE_integer('review_embedding_size', 384, 'Category id feature size')
...
def create_model(sess, config, item_cate_list, description_embedding):

  print(json.dumps(config, indent=4), flush=True)
  model = Model(config, item_cate_list, description_embedding)
...
def train():
  ...
  s3_key = f'cTLSAN_input_data/{FLAGS.dataset}_dataset.pkl'
  buffer = io.BytesIO()
  ...
  with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    model = create_model(sess, config, item_cate_list, description_embedding)
